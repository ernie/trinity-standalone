...
// Defined in vrcommon/vr_cvars.c - extern here to avoid duplicate symbol
...
static void R_Register( void )
{
	...
	r_subdivisions = ri.Cvar_Get( "r_subdivisions", "4", CVAR_ARCHIVE_ND | CVAR_LATCH );  // Mobile: lower detail curves
	...
	r_dynamiclight = ri.Cvar_Get( "r_dynamiclight", "0", CVAR_ARCHIVE );  // Mobile: disable for performance
	...
	r_dlightBacks = ri.Cvar_Get( "r_dlightBacks", "0", CVAR_ARCHIVE_ND );  // Mobile: disable for performance
	...
	ri.Cvar_CheckRange( r_flareCoeff, 0.1f, 99999, qfalse );
	...
	r_ext_max_anisotropy = ri.Cvar_Get( "r_ext_max_anisotropy", "4", CVAR_ARCHIVE_ND | CVAR_LATCH );  // Mobile: lower anisotropy
...
#ifdef USE_VULKAN
	// Initialize XR resources BEFORE creating pipelines.
	// This is needed both for soft restart (RE_Shutdown(0) + R_Init) and hard restart
	// (RE_Shutdown(2) + R_Init during game dir change). The VR layer's XR swapchains
	// remain valid across restarts, so we need to reinitialize the renderer's XR
	// resources (image views, framebuffers) to use them. RE_InitXRResources() handles
	// both fresh init and reinit cases.
	RE_InitXRResources();
...
void RE_WaitForRenderComplete( void ) {
	...
	ri.Printf( PRINT_ALL, "[WaitForRenderComplete] waiting on fence (cmd_index=%d)...\n", vk.cmd_index );
	result = vkWaitForFences( vk.device, 1,
		&vk.cmd->rendering_finished_fence,
		VK_TRUE, 1000000000 ); // 1 second timeout
	ri.Printf( PRINT_ALL, "[WaitForRenderComplete] fence returned %d\n", result );
